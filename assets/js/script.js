...
function updateParticlesColor(theme) {
    // Only update if particlesJS is loaded
    if (window.pJSDom && window.pJSDom.length > 0) {
        var color = theme === 'dark' ? '#ffffff' : '#222222';
        // Update color for all loaded particles instances
        window.pJSDom.forEach(function (particleInstance) {
            if (particleInstance.pJS && particleInstance.pJS.particles && particleInstance.pJS.particles.color) {
                particleInstance.pJS.particles.color.value = color;
                // Also update the color in the config if present
                if (particleInstance.pJS.particles.line_linked) {
                    particleInstance.pJS.particles.line_linked.color = color;
                }
                // Force redraw
                particleInstance.pJS.fn.particlesRefresh();
            }
        });
    }
}
